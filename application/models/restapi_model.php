...
ORDER BY `osb_transaction`.`id` DESC")->result();
        ...
        $query['admin'] = $this->db->query("SELECT `osb_transaction`.`id`,`osb_transaction`. `userto`,`osb_transaction`. `userfrom`,`osb_transaction`. `reason`,`osb_transaction`. `amount`,`osb_transaction`. `payableamount`,DATE(ADDTIME(`osb_transaction`.`timestamp`,'0 05:30:00')) AS `date`,DATE_FORMAT(STR_TO_DATE(ADDTIME(`osb_transaction`.`timestamp`,'0 05:30:00'), '%Y-%m-%d %H:%i:%s'), '%H:%i:%s') as `time`,`osb_transaction`. `requestid` ,`osb_request`.`approvalreason`,`osb_transaction`.`timestamp` as `timestamp` FROM `osb_transaction` LEFT OUTER JOIN `osb_request` ON `osb_transaction`.`requestid`=`osb_request`.`id` WHERE `osb_transaction`.`userfrom`=1 AND `osb_transaction`.`userto`='$user' ORDER BY `osb_transaction`.`id` DESC")->result();
